...
namespace Unity.MergeInstancingSystem.DebugWindow
{
    public class HierarchyItem: VisualElement, IDisposable
    {
        ...
        private InstanceItemData m_hlodData;
        private HierarchyItemData m_data;

        private InstanceDebugWindow m_window;
        private InstanceItem m_hlodItem;
        ...
        public HierarchyItem(InstanceDebugWindow window, InstanceItem hlodItem, InstanceItemData hlodData)
        {
            m_window = window;
            m_hlodItem = hlodItem;
            m_hlodData = hlodData;
        ...
        private void Update()
        {
            ...
            {
                isRendered = node.ExprectedState == InstanceTreeNode.State.Low ||
                             (node.ExprectedState == InstanceTreeNode.State.High && node.GetChildTreeNodeCount() == 0);
            }
